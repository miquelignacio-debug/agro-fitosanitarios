-- ============================================================
-- agro-fitosanitarios — Supabase Schema
...
INSERT INTO operadores (nombre, rut) VALUES
  ('Jorge Pino', '95768091');
-- Agregar los otros 2 operadores después
...
CREATE TABLE productos (
  ...
  tipo_funcion         text[],          -- FUNGUICIDA, INSECTICIDA, FITORREGULADOR, etc.
  formulacion          text,
  unidad_dosis         text,            -- lt/ha, kg/ha, g/100lt, etc.
  phi_dias             integer DEFAULT 0,
  rei_horas            integer DEFAULT 0,
  especies_autorizadas text[],          -- desde SAG: ['Cereza','Vid de mesa', ...]
  max_ia_descripcion   text,            -- texto libre de limitación IA
  activo               boolean DEFAULT true,
...
CREATE TABLE stock_movimientos (
  ...
  unidad                 text NOT NULL,          -- lt, kg, unidad, etc.
  fecha                  date NOT NULL,
  -- Solo entradas:
  documento_tipo         text CHECK (documento_tipo IN ('guia_despacho','factura')),
  documento_numero       text,
  proveedor              text,
  -- Solo salidas desde OT:
  ot_id                  uuid,                   -- se llena al finalizar la OT
  -- Transferencias:
...
-- Cuarteles de cada OT (puede ser más de uno)
...
-- Productos de cada OT
CREATE TABLE ot_productos (
  id                  uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ot_id               uuid NOT NULL REFERENCES ordenes_trabajo(id) ON DELETE CASCADE,
  producto_id         uuid NOT NULL REFERENCES productos(id),
  dosis_real          numeric(12,4) NOT NULL,
  dosis_unidad        text NOT NULL,         -- lt/ha, kg/ha, g/100lt
...
-- ── Función: número correlativo de OT por empresa ─────────────
...
-- Política: cualquier usuario autenticado puede leer y escribir
-- (para 2 admins — ajustar si se agregan operadores con acceso limitado)
